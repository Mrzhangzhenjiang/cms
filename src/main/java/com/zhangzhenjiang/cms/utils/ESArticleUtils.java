...
public class ESArticleUtils {
	private static Logger logger = Logger.getLogger(ESArticleUtils.class);
	...
	public static AggregatedPage<?> selectObjects(ElasticsearchTemplate elasticsearchTemplate, Class<?> clazz,
			Integer page, Integer rows, String fieldNames[], String value, Integer channelId, Integer categoryId) {
		...
		//判断栏目是否存在，并且分类不存在
		if (channelId != -1 && categoryId == -1) {
			queryBuilder = QueryBuilders.termQuery("channelId", channelId);
			System.out.println("channelId@@@@@@@@@@@@"+channelId);
		}
		//判断分类存在，栏目一定存在(不判断栏目)
		if (categoryId != -1) {
			queryBuilder = QueryBuilders.termQuery("categoryId", categoryId);
		...
		if (value != null && !"".equals(value)) {
			...
			// 创建queryBuilder对象
			QueryBuilder queryBuilderHighlight = QueryBuilders.multiMatchQuery(value, fieldNames);
			query = new NativeSearchQueryBuilder().withQuery(queryBuilderHighlight).withHighlightFields(highlightFields).withFilter(queryBuilder)
			...
			//判断分类存在，栏目一定存在(不判断栏目)
			if (categoryId == -1 && channelId==-1) {
				//查询热门文章
				queryBuilder = QueryBuilders.termQuery("hot", 1);
				System.out.println("都为-1");
			}
			// 没有查询条件的的时候，获取es中的全部数据 分页获取
			query = new NativeSearchQueryBuilder().withQuery(queryBuilder).withPageable(pageable).build();
